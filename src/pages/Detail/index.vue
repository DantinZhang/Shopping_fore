          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spu in spuSaleAttrList" :key="spu.id">
                <dt class="title">选择{{spu.saleAttrName}}</dt>
                <dd v-for="spuVal in spu.spuSaleAttrValueList" :key="spuVal.id" changepirce="0"
                  :class="{active:spuVal.isChecked === '1'}" @click="changeActive(spuVal,spu.spuSaleAttrValueList)">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="shopCarNum" @change="changeShopCarNum">
                <a href="javascript:" class="plus" @click="shopCarNum++">+</a>
                <a href="javascript:" class="mins" @click="shopCarNum>1?shopCarNum--:shopCarNum=1">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="sendDataToSql">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    //函数2：onchange事件:加入购物车数量输入框的内容改变时离开焦点触发此函数
    changeShopCarNum() {
      //拿到用户输入的值做校验
